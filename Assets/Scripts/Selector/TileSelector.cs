        ...
        void Update()
        {
            if (tileHighlight != null)
            {
                ...
                if (Physics.Raycast(ray, out hit, 100f, gridLayer))
                {
                    ...
                    {
                        tileHighlight.transform.SetPositionAndRotation(Geometry.PointFromGrid(gridPoint), Quaternion.Euler(flipX ? 180.0f : 0.0f, 90.0f * (int)piece.PieceDirection, flipZ ? 180.0f : 0.0f));
                    ...
                    if (Input.GetKeyUp("e"))
                    {
                        tileHighlight.transform.Rotate(flipX ? 180.0f : 0.0f, 90.0f, flipZ ? 180.0f : 0.0f, Space.Self);
                        if (dirIndex < 3)
                        {
                            dirIndex++;
                        }
